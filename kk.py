...
import time, random, sys, pafy, json, codecs, threading, glob, re, string, os, requests, subprocess, six, ast, pytz, urllib.request, urllib.parse, urllib.error, urllib.parse,antolib, subprocess, unicodedata, traceback
...
botStart = time.time()
#===============================================================================#
#line = LINE("EAmMNLnsUcNPw6x0Chfc./i/5rYKTg6f/CZ30PHlPRa.hdH1oPs0w79rGja5Xa6077dBDKoEErWLtyVyYVuRLd8=")
line = LINE("")
line.log("Auth Token : " + str(line.authToken))
lineMID = line.profile.mid
lineProfile = line.getProfile()
lineSettings = line.getSettings()
channel = Channel(line, line.server.CHANNEL_ID['LINE_TIMELINE'])
...
RfuBot=[lineMID]
Family=["ud88164955f26e175ba11a4eb56aae5d1",lineMID]
admin=['ud88164955f26e175ba11a4eb56aae5d1',lineMID]
adminMID="ud88164955f26e175ba11a4eb56aae5d1"
...
settings = {
    ...
    "Respontag":" 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖",
    "ResponPM":" 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖",
    "message":" 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖",
    "comment":" 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖",
    "Wc":" 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖",
    "bye":" 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖",
    ...
    try:
        ...
        if op.type == 13:
           ...
           if settings["autoCancel"]["on"] == True:
              if len(G.members) <= settings["autoCancel"]["members"]:
                  line.rejectGroupInvitation(op.param1)
                  print ("[13]ยกเลิกกลุ่มสมาชิกน้อยกว่า5คน")
        if op.type == 24:
...
#==============================================================================#
                ...
                if text.lower() == 'help':
                    ...
                    start = time.time()
                    line.sendMessage(to, "『𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖』")
...
#==============================================================================#
                ...
                elif text.lower() == 'เปิดออกกลุ่ม':
                    settings["autoLeave"] = True
                    line.sendMessage(to, "ออกกลุ่มอัตโนมัติ (เปิด) ใช้งาน📣")
                elif text.lower() == 'ปิดออกกลุ่ม':
                    settings["autoLeave"] = False
                    line.sendMessage(to, "ออกกลุ่มอัตโนมัติ (ปิด) ใช้งาน📣")
...
#=====================================================================================#                        
                   ...
                   if msg._from in admin:
                ...
                elif msg.text.lower().startswith("แปลอินโด "):
                    try:
                        proses = text.split(" ")
                        chivaree = text.replace(proses[0] + " ","")
                        r=requests.get("http://ariapi.herokuapp.com/api/trans?key=beta&to=in&text={}".format(chivaree))
                        data=r.text
                        data=json.loads(data)
                        chivaree2 = "{}".format(data["result"]["translated"])
                        line.sendMessage(to, str(chivaree2))
                    except Exception as error:
                                print(error)
                    ...
                    try:
                        proses = text.split(" ")
                        chivaree = text.replace(proses[0] + " ","")
                        r=requests.get("http://ariapi.herokuapp.com/api/trans?key=beta&to=th&text={}".format(chivaree))
                        data=r.text
                        data=json.loads(data)
                        chivaree2 = "{}".format(data["result"]["translated"])
                        line.sendMessage(to, str(chivaree2))
                                ...
                                print(error)
                elif msg.text.lower().startswith("แปลอังกฤษ "):
                    try:
                        proses = text.split(" ")
                        chivaree = text.replace(proses[0] + " ","")
                        r=requests.get("http://ariapi.herokuapp.com/api/trans?key=beta&to=en&text={}".format(chivaree))
                        data=r.text
                        data=json.loads(data)
                        chivaree2 = "{}".format(data["result"]["translated"])
                        line.sendMessage(to, str(chivaree2))
...
#-------------------------------------------------------------------------------------------------------------
                    ...
                    line.sendMessage(msg.to,"「✮ 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖 ✮」")                    
...
#-----------------------------------------------------------------------------------------------------------
                    ...
                    line.sendMessage(msg.to,"✮ 𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖 ✮" +datetime.today().strftime('%H:%M:%S')+ "™") 
...
#==============================================================================#                    
                  ...
                  if msg._from in admin:
                     ...
                     if spl in [""," ","\n",None]:
                         ...
                         settings["Wc"] = spl
                         line.sendMessage(msg.to, "『𝕨𝕖𝕖 𝕓𝕠??𝕝𝕚𝕟𝕖』\nตั้งข้อความตอบโต้เมื่อมีคนเข้ากลุ่ม\n\n[ {} ]".format(str(spl)))  
...
#=====================================================================================#
                        ...
                        if settings["checkContact"] == True:
                            if settings["lang"] == "JP":
                                line.sendMessage(to,"เปิดอ่านข้อความคอนแทคภายในห้อง...\n\n『𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖』", contentMetadata = {'AGENT_ICON': 'http://dl.profile.line-cdn.net/'+line.getContact(lineMID).pictureStatus, 'AGENT_NAME': 'Creator', 'AGENT_LINK': 'https://line.me/ti/p/Rvv5zHBLDO'})
                        else:
                            settings["checkContact"] = True
                            if settings["lang"] == "JP":
                                line.sendMessage(to,"เปิดอ่านข้อความคอนแทคภายในห้อง....\n\n『𝕨𝕖𝕖 𝕓𝕠𝕥𝕝𝕚𝕟𝕖』", contentMetadata = {'AGENT_ICON': 'http://dl.profile.line-cdn.net/'+line.getContact(lineMID).pictureStatus, 'AGENT_NAME': 'Creator', 'AGENT_LINK': 'https://line.me/ti/p/Rvv5zHBLDO'})
        ...
        if op.type == 26:
            ...
            if msg.toType == 0 or msg.toType == 1 or msg.toType == 2:
                if msg.toType == 0:
                    ...
                    to = receiver
                if msg.text in ["พี่วี","อือ","อื้อๆ","อ่า","ป๊าวี","ลุงวี"]:
                    user = line.getContact(msg._from)
                    sendMessageWithMention(msg.to, user.mid)
...
#===========ชุด API คำสั่งใช้งานบอทสิริเชนวี10 ========================== 
        if op.type == 26:
            ...
            if settings ["Api"] == True:
            	if msg.text in ["99","199","285","20000","20100","20200","30100","29900","39800","39850","39900"]:
                    line.kickoutFromGroup(receiver,[sender])
                    line.sendMessage(msg.to,"ตรวจพบคำสั่งนับเลข จำเป็นต้องนำออกเพื่อความปลอดภัยของสมาชิก (｀・ω・´)")
            if settings ["Api"] == True:
                if msg.text in ["ป๋าวี","วี","พี่วี","ตาวี","ทวดวี","วีจ๋า"]:
                    line.sendMessage(msg.to, "คาฟ..เรียกเค้าทำไมคิดถึงเค้าหรา😆😆")
            if settings ["Api"] == True:
            ...
            if settings["Api"] == True:
                if msg.text in ["งง","งงง",]:
                    line.sendText(msg.to,"ไม่ต้องงง เค้าน่ารักแบบนี้มาตั้งนานแล้ว😂😂")
            ...
            if settings ["Api"] == True:
                if msg.text in ["สวัดดี","หวัดดี","ดีจ้า","ดีครับ","ดี","สวัสดีครับ","สวัสดีค่ะ","ดีงับๆ","ดีงับ","หวัดดีงับๆ"]:
                    line.sendText(msg.to,"สวัสดีครับ ผมชื่อวีนะ ยินดีที่ได้รู้จัก")
            if settings ["Api"] == True:
